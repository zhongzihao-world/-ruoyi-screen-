...
const newPondItemInterval = (
  ...
  const fetchFn = async () => {
    try {
      const res = await customizeHttp(
        toRaw(requestDataPondItem.dataPondRequestConfig),
  ...
  const targetInterval =
    requestDataPondItem.dataPondRequestConfig.requestInterval;
  const targetUnit =
    requestDataPondItem.dataPondRequestConfig.requestIntervalUnit;
...
export const useChartDataPondFetch = () => {
  ...
  const addGlobalDataInterface = (
    ...
    // 组件对应的数据池 Id
    const requestDataPondId = targetComponent.request
      .requestDataPondId as string;
    // 新增数据项
  ...
  const initDataPond = (requestGlobalConfig: RequestGlobalConfigType) => {
    ...
    for (let pondKey of mittDataPondMap.keys()) {
      const requestDataPondItem = requestDataPond.find(
        item => item.dataPondId === pondKey,
      );
