...
import type { ChartEditStoreType } from '@/store/modules/chartEditStore/chartEditStore.d';

export const SCALE_KEY = 'scale-value';

export const useScale = (chartEditStore: ChartEditStoreType) => {
  const entityRef = ref();
  const previewRef = ref();
  const width = ref(chartEditStore.editCanvasConfig.width);
  const height = ref(chartEditStore.editCanvasConfig.height);
  ...
  // 屏幕适配
  onMounted(() => {
    switch (chartEditStore.editCanvasConfig.previewScaleType) {
