...
const minDistance = computed(() => {
  return chartSettingStore.getChartAlignRange;
});
...
const isSorption = (selectValue: number, componentValue: number) => {
  const isSorption =
    Math.abs(selectValue - componentValue) <= minDistance.value;
  return isSorption;
};
